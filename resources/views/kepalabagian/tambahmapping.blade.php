...
gedungs = gedungs.map(gedung => ({
    ...
    try {
        const response = await fetch('/kepalabagian/tambahmapping/', {
            method: 'POST',
            headers: {
...
async function updateGedung(gedung) {
    ...
    try {
        const response = await fetch(`/kepalabagian/tambahmapping/${gedung.id}`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': csrfToken,
            },
...
async function deleteGedung(id) {
    try {
        const response = await fetch(`/kepalabagian/tambahmapping/${id}`, {
            method: 'DELETE',
            headers: {
...
async function fetchGedungs(gedung) {
    try {
        const gambargedungId = {{ $gambargedungs->id }};
        const response = await fetch(`/kepalabagian/tambahmapping/${gambargedungId}/mapping`);
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
