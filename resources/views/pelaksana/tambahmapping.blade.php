    ...
    <script>
        ...
        function drawCanvas() {
            ...
            gedungs.forEach(gedung => {
                ...
                ctx.beginPath();
                ctx.rect(gedung.x - width / 2, gedung.y - height / 2, width,
                height); // Kotak dengan ukuran yang dapat diubah
                ...
                ctx.beginPath();
                ctx.arc(gedung.x + width / 2, gedung.y + height / 2, 5, 0, 2 * Math
                .PI); // Handle di sudut kanan bawah
        ...
        canvas.addEventListener('click', async (e) => {
            ...
            if (clickedGedung) {
                ...
                const confirmDelete = confirm(
                    `Apakah Anda yakin ingin menghapus ruangan "${clickedGedung.nama_ruangan}"?`);
                if (confirmDelete) {
                    try {
                        await deleteGedung(clickedGedung.id); // Hapus dari server
                        gedungs = gedungs.filter(gedung => gedung.id !== clickedGedung
                        .id); // Hapus dari array lokal
                        drawCanvas(); // Render ulang canvas
                    } catch (error) {
